...
if [[ $SUCCESS == 'true' ]]; then 
  if [[ $URL_OUTPUT != '' ]]; then
    ...
    ## VM Created! :rocket:
    $URL_OUTPUT"
  elif [[ $URL_OUTPUT == '' && $BITOPS_CODE_ONLY == 'true' && $BITOPS_CODE_STORE == 'true' ]]; then
    ...
    result_string="## BitOps Code generated. :tada:"
  elif [[ $URL_OUTPUT == '' && $TF_STACK_DESTROY != 'true' && $BITOPS_CODE_ONLY != 'true' ]]; then
  ...
  elif [[ $URL_OUTPUT == '' && $TF_STACK_DESTROY == 'true' && $TF_STATE_BUCKET_DESTROY != 'true' ]]; then
