...
rm "${BITOPS_ENVROOT}/generators/terraform.tfvars"
  ...
  cp "${BITOPS_ENVROOT}"/generators/aws/aws_default* "${BITOPS_ENVROOT}"/terraform/aws/rds/.
#fi
  ...
  cp "${BITOPS_ENVROOT}"/generators/aws/aws_default* "${BITOPS_ENVROOT}"/terraform/aws/efs/.
#fi
  ...
  cp "${BITOPS_ENVROOT}"/generators/aws/aws_default_tags* "${BITOPS_ENVROOT}"/terraform/aws/eks/.
#fi
if [ $(find "$BITOPS_ENVROOT/generators/." -iname "bitovi_*.tf" | wc -l) -gt 0 ]; then 
  ...
  mv "${BITOPS_ENVROOT}"/generators/aws/aws_default* "${BITOPS_ENVROOT}"/terraform/aws/ec2/.
fi 
...
find "${BITOPS_ENVROOT}/terraform/aws" -maxdepth 1 -type d -not -name "."  -path "${BITOPS_ENVROOT}/terraform/aws/*" | while read terraform_folder; do
  cat "${BITOPS_ENVROOT}/generators/aws/aws_variables.tf" >> $terraform_folder/variables.tf
done
