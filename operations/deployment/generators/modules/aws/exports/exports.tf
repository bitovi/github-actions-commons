...
resource "local_file" "aws_dotenv_tf" {
    ...
    content  = templatefile(format("%s/%s", abspath(path.module), "aws_dotenv_tf.tmpl"), {
      ...
      host_dir_string         = var.aws_efs_create || var.aws_efs_create_ha ? "$${var.aws_efs_ec2_mount_point}" : ""
      target_dir_string       = var.aws_efs_create || var.aws_efs_create_ha ? "$${var.docker_efs_mount_target}" : ""
    })
}
