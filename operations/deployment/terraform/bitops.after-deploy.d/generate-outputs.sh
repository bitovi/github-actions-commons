...
if [ "$BITOPS_TERRAFORM_COMMAND" != "destroy" ]; then
    ...
    echo "cat"
    cat /opt/bitops_deployment/bo-out.env| grep instance_public_dns | awk -F"\"" '{print $1}'
    cat /opt/bitops_deployment/bo-out.env| grep instance_public_dns | awk -F"\"" '{print $2}'
    echo "BITOPS_EC2_PUBLIC_IP $BITOPS_EC2_PUBLIC_IP"
    echo "cat"
    cat /opt/bitops_deployment/bo-out.env| grep instance_public_ip | awk -F"\"" '{print $1}'
    cat /opt/bitops_deployment/bo-out.env| grep instance_public_ip | awk -F"\"" '{print $2}'
echo -en "
