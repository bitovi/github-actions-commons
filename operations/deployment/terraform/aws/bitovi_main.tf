module "ec2" {
  ...
  common_tags                         = merge(local.default_tags,jsonencode(var.aws_ec2_additional_tags))
...
module "vpc" {
  ...
  # Others
  aws_resource_identifier     = var.aws_resource_identifier
  common_tags                 = merge(local.default_tags,jsonencode(var.aws_vpc_additional_tags))
}
