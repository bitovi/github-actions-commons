...
resource "aws_alb_listener" "http_redirect" {
  count             = var.aws_alb_redirect_enable && var.aws_certificates_selected_arn != "" ? !contains(local.alb_listen_port, 80) ? 1 : 0 : 0
  load_balancer_arn = aws_lb.vm_alb.arn
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_alb_listener" "http_forward" {
  count             = var.aws_alb_redirect_enable && !var.aws_alb_www_to_apex_redirect && var.aws_certificates_selected_arn == "" ? !contains(local.alb_listen_port, 80) ? 1 : 0 : 0
  load_balancer_arn = aws_lb.vm_alb.arn
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_alb_listener" "https_redirect" {
  count             = var.aws_alb_redirect_enable && var.aws_certificates_selected_arn == "" ? !contains(local.alb_listen_port, 443) ? 1 : 0 : 0
  load_balancer_arn = aws_lb.vm_alb.arn
  port              = "443"
  protocol          = "HTTPS"
  certificate_arn   = var.aws_certificates_selected_arn
