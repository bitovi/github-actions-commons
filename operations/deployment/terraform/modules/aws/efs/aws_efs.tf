locals {
  ...
  create_mountpoints  = var.aws_efs_create_ha || var.aws_efs_create_mountpoints ? true : false
  create_sg           = var.aws_efs_create || local.create_mountpoints ? true : false
}
...
resource "aws_efs_mount_target" "efs_mount_target_incoming" {
  count           = local.create_mountpoints ? length(local.aws_efs_subnets) : 0
  file_system_id  = data.aws_efs_file_system.efs.id
...
resource "aws_security_group_rule" "ingress_efs" {
  count             = local.create_sg && var.aws_efs_ingress_allow_all ? 1 : 0
  type              = "ingress"
...
resource "aws_security_group_rule" "ingress_efs_extras" {
  count                    = local.create_sg ? length(local.aws_efs_allowed_security_groups) : 0
  type                     = "ingress"
